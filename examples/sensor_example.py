...
from PyQt5.QtWidgets import QDoubleSpinBox, QLabel, QVBoxLayout, QWidget
...
class LiveSensorViewer:
    def __init__(self, sensor_update_interval=25):
        ...
        self.my_graph = self.plot_widget.addPlot(
            axisItems={"bottom": pg.DateAxisItem()}
        )
        # self.plot_widget.addPlot()
        # self.plot_widget.addItem(TextItem(), row=1, col=0)
        self.my_graph.showGrid(x=True, y=True, alpha=0.25)
        self.my_graph.addLegend()

        # Curves to be drawn on the graph
        self.curve_position_raw = self.my_graph.plot(
            self.log_time,
            self.log_position_raw,
            name="Position raw (mm)",
            pen=pg.mkPen(color="#525252"),
        )
        self.curve_position_moving_avg = self.my_graph.plot(
            self.log_time,
            self.log_position_raw,
        ...
        self.my_dialog = QWidget()
        self.verticalLayout = QVBoxLayout(self.my_dialog)

        self.my_label = QLabel()
        self.verticalLayout.addWidget(self.my_label)
        self.my_label.setText("Current sensor position:")

        self.my_sensor_value = QDoubleSpinBox()
    ...
    def read_position(self, sensor_obj, update_interval):
        print(
            "Thread ={}          Function = read_position()".format(
    ...
    def handle_sensor_data(self, new_pos, new_pos_time):
        print(
            "Thread ={}          Function = handle_sensor_data()".format(
                threading.current_thread().name
            )
        )
    ...
    #     print(
    #         "Thread ={}          Function = calculate_moving_avg()".format(
    #             threading.currentThread().getName()
    #         )
    #     )
    #     # Get moving average of the position
    #     self.log_position_moving_avg = bn.move_mean(
    #         self.log_position_raw, window=self.moving_avg, min_count=1
    #     )

    def update_graph(self):
        print(
            "Thread ={}          Function = update_graph()".format(
                threading.current_thread().name
            )
        )
        self.curve_position_raw.setData(self.log_time, self.log_position_raw)
        self.curve_position_moving_avg.setData(self.log_time, self.log_position_raw)
