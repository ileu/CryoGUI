...
class System_service():
    ...
    def errorNumberToRecommendation(self, language, err_num):
        """
        ...
        err_num:   interger: Error code to translate

        Returns
        -------
        value_string1: string: Error recommendation (currently returning an int = 0 until we have recommendations)
        """
        response = self.device.request(self.interface_name + "." + "errorNumberToRecommendation", [language, err_num])
        self.device.handleError(response)
        return response['result'][1]

    def errorNumberToString(self, language, err_num):
        """
        ...
        """
        response = self.device.request(self.interface_name + "." + "errorNumberToString", [language, err_num])
        return response['result'][1]
